...
def createRandomNBody(n):
    return np.array([Body(Vector(1e7*random.randint(-5,5),1e7*random.randint(-5,5)),Vector(random.randint(-50,50),random.randint(-50,50)),1e24*random.randint(1,10)) for i in range(n)])
...
def animate(bodies,time_step,total_time,boundaries:tuple):
    ...
    animPlot = [plt.plot([],[],".-")[0] for _ in range(len(bodies))]
